...
impl DB {
    fn new(db_file: &str, remove_db_data: bool) -> Result<DB, String> {
    ...
    fn insert(&mut self, headers: &Headers, addrs: &[StringRecord]) {
        ...
        let mut errors = {
            ...
            addrs
                ...
                .filter_map(|x| {
                    let obj = Address {
                        lat: get_with_headers!(headers, "lat", x)
                            .and_then(|x| f64::from_str(x).ok()),
                        lon: get_with_headers!(headers, "lon", x)
                            .and_then(|x| f64::from_str(x).ok()),
...
pub fn import_addresses<P: AsRef<Path>>(db_file_name: &str, folder: P, remove_db_data: bool) -> DB {
    let mut db = DB::new(db_file_name, remove_db_data).expect("failed to create DB");
    visit_dirs(folder, &mut db);
    db
}
